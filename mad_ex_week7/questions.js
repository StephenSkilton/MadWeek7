...
function abbreviateName(name) {
  ...
  const reg = /^\s*(\w+\b)\s*(\b\w+\b)?\s*(\b\w+)\s*$/;
return name.replace(reg, (_, f, m, l) =>
m ? `${l}, ${f[0]}. ${m[0]}.` : `${l}, ${f[0]}.`
);
}
 ...
 */
function invokeWithApplyAndCall(func, context, argsArray) {
  return [func.call(context, ...argsArray), func.apply(context, argsArray)];
}
